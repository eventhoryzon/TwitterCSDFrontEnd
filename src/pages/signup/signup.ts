...
export class Signup {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,public userregformbuilder : FormBuilder,public authService: AuthService, public loadingCtrl: LoadingController, private toastCtrl: ToastController) {
  ...
  doSignup() {
    ...
    this.authService.signup(this.userregform).then((result) => {
      ...
      this.presentToast('User Credententials are Saved- Please Login.');
      this.navCtrl.pop();
    }, (err) => {
      this.loading.dismiss();
