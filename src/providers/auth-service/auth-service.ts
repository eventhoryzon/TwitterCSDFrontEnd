...
export class AuthService {
  ...
  constructor(public http: Http,isLoggedin,AuthToken,LOCAL_TOKEN_KEY) {
    this.http = http;
    this.LOCAL_TOKEN_KEY = "User Token";
  ...
  login(credentials){
    ...
    return new Promise((resolve, reject) => {
        ...
        this.http.post(apiUrl+'loginAuthenticate', JSON.stringify(credentials) , {headers: headers})
  ...
  signup(data) {
    return new Promise((resolve, reject) => {
        ...
        this.http.post(apiUrl+'addNewUser', JSON.stringify(data.value), {headers: headers})
          .subscribe(res => {
            resolve(res.json());
          }, (err) => {
            reject(err);
   ...
   getinfo() {
        return new Promise(resolve => {
            ...
            this.http.get(apiUrl+'getinfo', {headers: headers}).subscribe(data => {
